...
for i in range(num_users):
	phone_num = "(" + selectRandomFromList(area_codes) + ")" + genNumID(7)
	phone_nums.append(phone_num)
	tower = selectRandomFromList(locations)
	hour = selectRandomFromList(hour_of_day_distr)
...
for i in range(num_calls):
	...
	if randint(0, 100) < 80:
		tower = profile[0]
		hours = int(profile[1].sample) / 2
	else:
		tower = selectRandomFromList(locations)	
		hours = int(selectRandomFromList(hour_of_day_distr).sample) / 2
